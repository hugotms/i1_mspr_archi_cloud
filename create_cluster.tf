...
resource "null_resource" "ansible" {
  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u svcansible -e client_id=${random_string.new_client_id} -i ${local_file.inventory.filename} --private-key ${var.ansible_private_key} ./ansible/play_configure_cluster.yml"
  }
}
...
output "client_id" {
  ...
  value = "Cluster IP for client ${random_string.new_client_id} is ${vsphere_virtual_machine.master.default_ip_address}"
}
